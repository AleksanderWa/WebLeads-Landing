...
faq:
  ...
  - question: "How do I verify an email address before sending?"
    answer: "Most email finder tools include built-in verification. If yours does not, standalone verifiers like Bouncer, ZeroBounce, and NeverBounce all offer free credits to start. Verification works via an SMTP ping to the mail server and no actual email is sent to the address."
  - question: "Is it legal to use business owner email addresses for cold outreach?"
    answer: "Cold outreach to business email addresses is generally permitted under CAN-SPAM (US) and CASL (Canada) as long as you include an unsubscribe option and your business name and address. GDPR in the EU is stricter for B2C, but business-to-business outreach has more latitude. Always include an opt-out mechanism and honor removal requests."
...
See the [find email addresses guide](/find-email-addresses) for more on this approach.
...
1. Find the person on LinkedIn (look for owner, founder, or managing partner)
2. Note their first name, last name, and company domain
3. Guess the email format (first@company.com, firstname.lastname@company.com, f.lastname@company.com, etc.)
4. Use an SMTP verifier to confirm which format is correct before sending
...
**How it works:**
1. Enter the company domain (e.g., acme.com)
2. Hunter shows emails it has found on that domain
3. It also surfaces the email pattern used (e.g., {first}.{last}@company.com)
4. Use that pattern to construct the owner's likely email
...
See also: [email finder tool comparison](/tools/email-finder-tool)
...
**How to check:**
1. Go to who.is or lookup.icann.org
2. Enter the business domain
3. Look for a registrant email in the results
